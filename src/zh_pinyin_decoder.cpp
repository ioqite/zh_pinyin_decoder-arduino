 ...
 * 
 * @note : 
 * 在静态(内部)函数中，我们认为输入字符串是有效的，并且不对其进行检查。
 * 不过在公共函数的开头均检查了输入字符串是否有效 (调用“chk_valid_string”函数)。
 * In static function, we consider input string valid and not check it.
...
static bool FS_is_begin = false;
static File file_code_table;
static File file_word_dict;
 ...
 */
static __split_method_t* mnode_init(void) {
    __split_method_t* m = (__split_method_t*)zh_buffer_malloc(sizeof(__split_method_t));
    if (m == NULL) {
...
static __split_method_list_t* mlist_init(void) {
    __split_method_list_t* m_list = (__split_method_list_t*)zh_buffer_malloc(sizeof(__split_method_list_t));
    if (m_list == NULL) {
...
static uint8_t pinyin_dfs(__split_method_list_t* m_list, const char* str, uint8_t* spm, uint8_t m_wt, uint8_t depth) {
    ...
    __code_index_t* codex = (__code_index_t *)(&code_index[idx1]);
    if (codex->table_length == 0) {
        g_word_match_number++; return 1;
    }

    uint8_t sl = (strlen(str) - start_idx > (uint)MAX_WORD_CODE_LENGTH) ? (uint)MAX_WORD_CODE_LENGTH : strlen(str) - start_idx;   /* strlen (left_str) */
    uint8_t res = 1;
    if (sl > 1) {
#if (USE_ZH_HASH_BOOST == 0)
        for (int i = 0; i < codex->table_length; i++) {
    ...
    if (res) { /**  no match found for len >=2, we can just split it in 1*/
        uint8_t tmp = (codex->table_length > (uint8_t)ZH_PINYIN_SIGNLE_SEARCH_DEPTH) ? (uint8_t)ZH_PINYIN_SIGNLE_SEARCH_DEPTH : codex->table_length;
...
static __word_block_t* wordblock_init(uint8_t type) {
    __word_block_t* w = (__word_block_t *)zh_buffer_malloc(sizeof(__word_block_t));
    if (!w) return NULL;
    w->type = type;
    switch(type){
...
static cJSON* cjson_parse_piece(char* buf, uint32_t* bytes_left) {
    ...
    for (pend = (uint8_t *)(buf + ZH_WORD_DICT_BUFFER_SZ - 2); pend != pstart; pend--) {
        if (*pend == ']') {
    ...
    cJSON* item = cJSON_Parse((char *)pstart);
    memcpy(pend, conn, 2);
...
static __word_block_t* wordblock_reshape(__word_block_t* w_res, uint8_t search_state) {
    ...
    if (search_state == WORD_SEARCH_STATE_CODE_PREC_MATCH && w_res->num.code_nbr > ZH_WORD_CODE_DISP_NUM) {
        ...
        uint8_t* new_buf1 = (uint8_t *)zh_buffer_malloc(buf1_sz + 1);
...
static __word_block_t* word_dict_search(const char* str, __split_method_list_t* m_list){
    ...
    char* res_str = (char *)zh_buffer_malloc(MAX_WORD_BLK_BUFFER_SZ);
    ...
    file_word_dict.seek(word_dict_offset[str[0] - 'a'], SeekSet);
    ...
    while (m_list->num > 0 && read_buf_num < ZH_WORD_MAX_BUFFER_READ) {
        ...
        if (!file_word_dict.available()) break;
        file_word_dict.read(word_dict_buffer + bytes_left, ZH_WORD_DICT_BUFFER_SZ - bytes_left);
        // if (feof(fp)) break;
    ...
    if (tmp > 0) {
        uint8_t* buf = (uint8_t *)zh_buffer_malloc(tmp + 1);
        if (buf == NULL) {
...
uint8_t zh_match_code_prec(const char* str, char* res_str, uint8_t num, uint8_t* br){
    ...
    if (get_match_idx(str, &match_idx, 0, NULL, (int8_t*)&v_br) || match_idx < 0) {
        return 1;
    }
    uint8_t idx = str[0] - 'a';
    const __code_index_t* codex = (&code_index[idx]);
    ...
    file_code_table.seek(read_loc, SeekSet);
    file_code_table.read((uint8_t*)res_str, read_length);
...
uint8_t zh_match_code_vague(const char* str, char* res_str, uint8_t num, uint8_t* br) {
    if (res_str == NULL || chk_valid_string(str)) return 1;
    int8_t mid = 0;
    uint8_t* v_idx = (uint8_t*)zh_buffer_malloc(num);
    if (v_idx == NULL) {
    ...
    if (chars_left > 0 && mid >= 0) {
        uint8_t match_num = (chars_left < codex->code_table_num[mid]) ? chars_left : codex->code_table_num[mid];
        match_num = (match_num < ZH_VAGUE_MATCH_HEAD_DEPTH) ? match_num : ZH_VAGUE_MATCH_HEAD_DEPTH;
        ...
        file_code_table.seek(read_loc, SeekSet);
        file_code_table.read((uint8_t*)(res_str + (size_t)chars_left * 3), (size_t)match_num * 3);
        // fseek(fp, read_loc, SEEK_SET);
    ...
    if (chars_left > 0 && v_br > 0) {
        ...
        for (int i = 0; i < v_br && cur_types < match_types; i++, cur_types++) {
            ...
            uint8_t match_num = (chars_left < codex->code_table_num[index]) ? chars_left : codex->code_table_num[index];
            match_num = (match_num < match_depth) ? match_num : match_depth;
            chars_left -= match_num; br_read += match_num;
            ...
            file_code_table.seek((long)read_loc, SeekSet);
            file_code_table.read((uint8_t*)(res_str + 3 * chars_left), read_length);
    ...
    if (chars_left > 0 && mid>=0 && codex->code_table_num[mid] > ZH_VAGUE_MATCH_HEAD_DEPTH) {
        uint8_t match_num = (chars_left < codex->code_table_num[mid] - ZH_VAGUE_MATCH_HEAD_DEPTH) ? chars_left : codex->code_table_num[mid] - ZH_VAGUE_MATCH_HEAD_DEPTH;
        chars_left -= match_num; br_read += match_num;
...
#if (USE_FAT_FS == 1)
    file_code_table = FFat.open(ZH_CODE_TABLE_FILE_NAME, FILE_READ);
#elif (USE_LITTLE_FS == 1)
